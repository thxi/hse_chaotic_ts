...
class TSProcessor:
    def __init__(self, points_in_template: int, max_template_spread: int):
        ...
        self._template_shapes: np.ndarray = self._templates[:, 1:] - self._templates[:, :-1]  # k1, k2, ...
    ...
    def pull(self, steps: int, eps: float, n_trajectories: int, noise_amp: float, prev_result: np.ndarray = None,
             random_seed=1,
        ...
        if prev_result is None:
            ...
            self._time_series[self._original_size: self._original_size + steps] = prev_result
            # update the _training_vectors
            # self.fit(self._time_series, refit=True)

        def get_trajectory_forecast(i: int, time_series: np.ndarray, training_vectors: np.ndarray,
                                    template_shapes: np.ndarray, original_size: int, x_dim: int,
            ...
            for j in range(steps):
                ...
                last_vectors = (time_series[:original_size + j][
                    np.cumsum(-template_shapes[:, ::-1], axis=1)[:, ::-1]])  # invert templates

                distance_matrix = _calc_distance_matrix(training_vectors, last_vectors, y_dim,
                ...
                forecast_point = _freeze_point(points, 'mean') + np.random.normal(0, noise_amp)
                ...
                new_training_vectors = (
                    time_series[:original_size + 1 + j]
                    [np.hstack((np.cumsum(-template_shapes[:, ::-1], axis=1)[:, ::-1]
        ...
        results = Parallel(n_jobs=n_jobs)(
            delayed(get_trajectory_forecast)(i, self._time_series, self._training_vectors,
    ...
    def cluster_sets(self, forecast_sets: np.ndarray, dbs_eps: float, dbs_min_samples: int):
        ...
        for i in range(len(forecast_sets)):
            ...
            curr_set = curr_set[~np.isnan(curr_set)]  # filter nans for trajectories
    ...
    def get_quantile_prediction(self, forecast_sets: np.ndarray, max_err: float, alpha=0.05):
        """
        Скластеризировать полученные в результате пулла множества прогнозных значений.
        ...
        for i in range(len(forecast_sets)):
            ...
            print(i, q1, q2, q2 - q1)
            if q2 - q1 <= max_err:
                predictions[i] = np.mean(forecast_sets[i][(forecast_sets[i] > q1) & (forecast_sets[i] < q2)])

        return predictions


def _calc_distance_matrix(training_vectors: np.ndarray, test_vectors: np.ndarray, y_dim: int, mask: np.ndarray,
    ...
    distance_matrix = ((training_vectors[mask, :, 0] - np.repeat(test_vectors[:, 0], y_dim + steps)
                        .reshape(-1, y_dim + steps)) ** 2
                       + (training_vectors[mask, :, 1] - np.repeat(test_vectors[:, 1], y_dim + steps)
                          .reshape(-1, y_dim + steps)) ** 2
                       + (training_vectors[mask, :, 2] - np.repeat(test_vectors[:, 2], y_dim + steps)
                          .reshape(-1, y_dim + steps)) ** 2) ** 0.5

    return distance_matrix


def _freeze_point(points_pool: np.ndarray, how: str, dbs_eps: float = 0.0, dbs_min_samples: int = 0) -> float:
    """
